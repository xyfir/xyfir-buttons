...
export default async function(url, files) {
  if (!Array.isArray(files)) throw 'Could not read repository files';
  ...
  if (manifest) {
    ...
    if (typeof manifest.button.styles == 'object')
      manifest.button.styles = JSON.stringify(manifest.button.styles);
  ...
  // Build array of files to download
    ...
    download = files.filter(f => f.type == 'file').map(f => f.name);
  }
  ...
  if (manifest) return Object.assign(manifest.button || {}, { script });
  else return { script };
}
