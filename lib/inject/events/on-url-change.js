...
export default function(event) {
  ...
  const container = document.querySelector('.xyfir-buttons-container');

  // Active preset still matches the current url
  if (presets.current.id && RegExp(presets.current.uriMatch).test(location)) {
    // Render or destroy buttons in active preset
    presets.current.buttons.forEach(button => {
      const match = RegExp(button.uriMatch).test(location);
