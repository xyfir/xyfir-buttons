/**
 * Returns the active preset for the tab's current uri.
...
export default function(event, data) {
  ...
  chrome.p.storage.local
    ...
    .then(res => {
      if (!Array.isArray(res.presets)) throw 'No presets to get';
      ...
      // Get id / uriMatch for all presets that match the current uri
      matches = res.presets.filter(preset => {
        return RegExp(preset.uriMatch).test(location.href);
      });

      // If tab's active preset or data.usePreset doesn't exist or doesn't
      // match current uri, get the first preset that matches the current uri
      if (!match || !RegExp(match.uriMatch).test(location.href))
        match = matches[0];

      if (!match) throw 'No presets to get';
