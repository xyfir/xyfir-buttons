...
export default function(event, data) {
  ...
  let match = {}, matches = [], localPresetData = {};
  ...
  chrome.p.storage.local
    ...
    .then(res => {
      ...
      if (!res[tabKey]) res[tabKey] = { activePreset: -1 };
      ...
      if (data.usePreset == match.id)
        chrome.storage.local.set({ [tabKey]: { activePreset: match.id } });

      // Get full data for preset
      return chrome.p.storage.local.get([
        'preset_' + match.id, 'localpreset_' + match.id
      ]);
    })
    .then(res => {
      ...
      localPresetData = res['localpreset_' + match.id] || { buttons: [] };
    ...
    .then(res => {
      ...
      match.buttons = match.buttons.map(b1 => {
        ...
        // Local preset styles override preset creator styles override button
        // creator styles (which then override xyButtons button styles)
        b3.styles = JSON.parse(b3.styles),
        b2.styles = JSON.parse(b2.styles),
        b1.styles = JSON.parse(b1.styles);
        Object.assign(b2.styles, b1.styles, b3.styles);

        b3.styles = b1.styles = b2.styles;

        return Object.assign(b2, b1, b3);
      });
